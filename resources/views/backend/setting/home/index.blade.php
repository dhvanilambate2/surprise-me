            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <h5 class="d-flex align-items-center">Our Team Data &nbsp;
                        <input type="checkbox" id="switch" name="team_section" data-section-name="team_section" class="toggle_input" {{ ($team_section->status == 1)? 'checked' : '' }}/>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="">{{ $item->post}}</td>
                                <td class="text-center">
                                    <input class="current-project-checkbox" onchange="updateCurrentProject(this, {{ $item->id }}, this.checked)" id="{{ $item->id }}"  type="checkbox" {{ ($item->current_four == '1')? "checked" : '' }}>
                                </td>
                                <td class="text-nowrap">
                                    {{-- <a href="{{ route('blog.details', ['id' => $item->id]) }}"><button
                                            class="btn btn-shadow-secondary btn-sm px-3">Preview </button></a> --}}
                                    <a href="{{ route('setting_home.team_edit', $item->id) }}"><button
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <form action="{{ route('setting_home.team_destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="d-flex align-items-center">Client Review Data &nbsp;
                        <input type="checkbox" id="switchReview" name="review_section" data-section-name="review_section" class="toggle_input" {{ ($review_section->status == 1)? 'checked' : '' }}/>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    {{-- <a href="{{ route('blog.details', ['id' => $item->id]) }}"><button
                                            class="btn btn-shadow-secondary btn-sm px-3">Preview </button></a> --}}
                                    <a href="{{ route('setting_home.review_edit', $item->id) }}"><button
                                            class="btn btn-shadow-primary btn-sm">Edit </button></a>
                                    <!-- Button trigger modal -->

                                    <button class="btn text-danger px-2" type="button" data-toggle="modal"
                                        data-original-title="test" data-target="#exampleModalReview{{$item->id}}"><i
                                            data-feather="trash-2"></i></button>
                                    <div class="modal fade" id="exampleModalReview{{$item->id}}" tabindex="-1" role="dialog"
                                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    {{-- Form for calling the destroy method --}}
                                                    <form action="{{ route('setting_home.review_destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger">Delete</button>
...
<script>
    ...
    function updateCurrentProject(selectElement, itemId, isChecked)
    {
        ...
        // Make an Ajax call
        $.ajax({
            url: '{{ route("setting_home.update_current_four") }}',  // Replace with your actual route
            method: 'POST',
            data: {
                _token: csrfToken, 
                id: itemId,
