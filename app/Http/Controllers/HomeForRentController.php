...
class HomeForRentController extends Controller
{
    ...
    {
        $homeDetails = HomeDetails::where('home_for', 'rent')->get();
        $homeDetails_publish = HomeDetails::where('home_for', 'rent')->where('status','publish')->get();
        $homeDetails_review = HomeDetails::where('home_for', 'rent')->where('status','review')->get();
        $homeDetails_draft = HomeDetails::where('home_for', 'rent')->where('status','draft')->get();
        $homeDetails_archive = HomeDetails::where('home_for', 'rent')->where('status','archive')->get();
        $homeDetails_rented = HomeDetails::where('home_for', 'rent')->where('status','rented')->get();
        return view('backend.rent.index', ['homeDetails' => $homeDetails, 
                                           'homeDetails_publish' => $homeDetails_publish,
                                           'homeDetails_review' => $homeDetails_review,
                                           'homeDetails_rented' => $homeDetails_rented,
                                           'homeDetails_archive' => $homeDetails_archive,
                                           'homeDetails_draft' => $homeDetails_draft]);
    }
    ...
    public function getHomeDetails(Request $request)
    {
        ...
        switch ($tab) {
            case 'publish':
                $homeDetails = HomeDetails::where('home_for', 'rent')->where('status','publish')->get(); 
                break;
            case 'archive':
                $homeDetails = HomeDetails::where('home_for', 'rent')->where('status','archive')->get(); 
                break;
            case 'rent':
                $homeDetails = HomeDetails::where('home_for', 'rent')->where('status','rented')->get(); 
                break;
            case 'draft':
                $homeDetails = HomeDetails::where('home_for', 'rent')->where('status','draft')->get(); 
                break;
            case 'review':
                $homeDetails = HomeDetails::where('home_for', 'rent')->where('status','review')->get(); 
                break;
            default:
                $homeDetails = [];
                break;
        ...
        $tableContent = view('backend.rent.table_content', ['homeDetails' => $homeDetails,'currentTab' => $tab])->render();
    ...
    public function store(Request $request)
    {
        ...
        $homeDetails->titles = json_encode($titles);
        $homeDetails->save();
    ...
    public function show(string $id)
    {
        ...
        return view('backend.rent.show', compact('homeDetails'));
    }
    ...
    public function edit(string $id)
    {
        ...
        // Load the associated media (brochures and images)
        $homeDetails->loadMedia('brochures', 'images');
        return view('backend.rent.edit', compact('homeDetails'));
    }
    ...
    public function destroy(string $id)
    {
        ...
        // Redirect or respond as needed
        return redirect()->route('home_for_rent.index')->with('success', 'The Home for Rent has been Deleted successfully.');
    }
